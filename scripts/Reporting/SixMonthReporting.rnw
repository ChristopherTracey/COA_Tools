...
if(length(unique(missingCompare$difference))==1&unique(missingCompare$difference[1])==0){
  ...
  cat("Compared to the previous six months, there are",words(abs(sum(missingCompare$difference))),"more SGCN in this data update as the result of these species being added to the various databases queried by this tool.")
} else {
